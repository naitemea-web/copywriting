...
- [x] F6 ③ `pages/Quiz`에 `buildDeck()` 연동: 카테고리 정답률 < 0.8 블록 / 이상 혼합 출제
- [x] F7 ⑤ 세션 페이싱: 8~12분 타이머 → 종료 넛지("자고 내일 복습") 노출
- [x] F9 `pages/Progress.tsx`: 숙련도(박스 분포)·보정 리포트·배지·연속일 표시
- [x] 배지 규칙: 첫 발/도감 탐험가/카피 설계자/메타인지 달인/3일 연속 해금 로직
- [x] 📱 반응형 폴리시: 모바일 우선 그리드·빈 상태(빈 복습 큐)·즉각 피드백
- [x] 접근성: 포커스 링·터치 44px+·시맨틱 버튼 (라이트하우스 정밀 점검은 추후)
- [~] 정적 빌드 `npm run build` ✓ / 배포는 호스팅 계정 연결 필요(미진행)
