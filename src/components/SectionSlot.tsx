  ...
  return (
    ...
    >
      <div className="flex items-center gap-xs">
        <span className="caption text-ink/60">{section.order}</span>
      ...
      <p className="caption text-ink/65">{section.goal}</p>
      <button
        type="button"
        onClick={onTap}
        className={`min-h-[44px] rounded-md border border-dashed text-body-sm font-500 ${
          placed ? 'border-primary bg-surface-soft' : 'border-hairline text-ink/60'
        }`}
      >
        {placed ? `${placed.icon} ${placed.name}` : '여기에 카드 놓기'}
      </button>
    </div>
  );
}
